...
Phase 4 focuses on using Wazuh as the centralized SIEM/XDR platform for security-event monitoring, detection analysis, alert investigation, and SOC analyst workflows.

Wazuh receives security telemetry from both Windows and Linux endpoints and provides a centralized interface for reviewing and investigating security activity.
...
- Validate connected Wazuh agents
- Review centralized Windows telemetry
- Review centralized Linux telemetry
- Analyze Wazuh security alerts
- Investigate Wazuh rule IDs and severity levels
- Review authentication-failure detections
- Analyze MITRE ATT&CK mappings
- Generate controlled security events
- Filter and search Wazuh Discover
- Correlate endpoint activity
- Document SOC analyst investigation workflows

---

## Wazuh Service Validation

The Wazuh security server hosts the centralized monitoring infrastructure.

The following services will be validated:

- `wazuh-manager`
- `wazuh-indexer`
- `wazuh-dashboard`

Service validation confirms that the SIEM/XDR infrastructure is operational before alert analysis begins.
...
## Connected Agent Validation

The Windows and Ubuntu endpoints will be verified as active Wazuh agents.

Expected monitored systems:

| Endpoint | Network | Role |
|---|---|---|
| SOC-Windows11 | Security LAN | Windows enterprise endpoint |
| SOC-Ubuntu | DMZ | Linux endpoint / controlled target |

Agent validation confirms that endpoint telemetry can be centrally monitored.
...
Wazuh Discover will be used to analyze endpoint security events using the:

`wazuh-alerts-*`

index pattern.

Security events will be reviewed using information such as:
...
Controlled failed-authentication activity generated on the Ubuntu endpoint will be investigated in Wazuh.

Previously validated telemetry includes:
...
Windows telemetry will be analyzed through the centralized Wazuh environment.

Relevant Windows telemetry includes:
...
The objective is to determine how Windows endpoint activity is parsed, categorized, and displayed by Wazuh.

---

## Wazuh Rule Analysis

Security alerts will be reviewed to understand how Wazuh applies detection rules.
...
This demonstrates the difference between raw endpoint telemetry and security alerts generated by Wazuh detection logic.

---

## MITRE ATT&CK Mapping

Where available, Wazuh detections will be mapped to MITRE ATT&CK tactics and techniques.
...
MITRE ATT&CK mapping provides additional context for understanding the behavior represented by a security alert.

---

## SOC Analyst Investigation Workflow

The Phase 4 investigation process will follow:
