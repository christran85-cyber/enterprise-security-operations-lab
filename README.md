...
Phase 13 focused on building a Python-based security automation workflow that processes security events from Suricata and Wazuh, generates structured security reports, and automatically sends SOC email notifications when qualifying Wazuh alerts are detected.
...
A structured JSON dataset was created to represent security events generated by Suricata and Wazuh.
...
[
  ...
  {
    "timestamp": "2026-09-14T20:16:32",
    "source_ip": "10.10.10.103",
    "destination_ip": "10.50.20.100",
    "source": "Suricata",
    "event_type": "HTTP Probe",
    "severity": "Low",
    "description": "HTTP probing activity detected"
  },
  {
    "timestamp": "2026-09-14T20:20:45",
    "source_ip": "10.50.20.100",
    "destination_ip": "10.10.10.102",
    "source": "Wazuh",
  ...
  {
    "timestamp": "2026-09-14T20:20:18",
    "source_ip": "10.10.10.103",
    "destination_ip": "10.10.10.102",
    "source": "Wazuh",
    "event_type": "File Integrity Change",
    "severity": "Low",
    "description": "Monitored file modification detected"
  }
]
```

The JSON structure was validated before being processed by the Python automation.
...
A Python script named `analyze_events.py` was developed to automatically process the JSON security-event dataset.

The script:

- Loads JSON security events
- Counts events by security source
- Counts events by severity
- Displays event timestamps
- Displays source and destination IP addresses
- Identifies security events requiring analyst attention
...
for event in events:
    ...
    print(
        f"{timestamp} | {source} | {severity} | "
        f"{event_type} | {source_ip} -> {destination_ip}"
    )
...
The automation successfully processed five security events:
...
The Python analysis output was exported into a persistent security report.
...
provides a reusable security-event summary that can be reviewed by a SOC analyst or incorporated into additional automation.

![Phase 13 Security Report](images/phase13-security-report.png)

---

# 4. CSV Security Report Automation

A second Python script named `export_csv.py` was created to automatically convert the JSON security-event data into CSV format.

This allows the security-event information to be imported into spreadsheets, dashboards, reporting systems, or additional analysis tools.
...
print("CSV security report created successfully.")
...
The resulting CSV was validated with:

```bash
cat phase13-security-events.csv
```
...
# 5. Automated SOC Email Notification

The next stage extended the Python automation into the live Wazuh environment.

The objective was to automatically notify a SOC analyst when Wazuh generates a qualifying security alert.

Rather than embedding an email account password inside the Python source code, the final implementation uses:

```text
Gmail API + OAuth 2.0
```

This provides API-based authentication while keeping authentication secrets separate from the Python source code.

---

# 6. Gmail API Configuration

A dedicated Google Cloud project was configured for the SOC security automation workflow.
...
The Gmail API was enabled to allow the Python automation to submit email notifications.
...
OAuth 2.0 was configured to authorize the security automation application.

The Gmail API permission used by the automation was:

```text
https://www.googleapis.com/auth/gmail.send
```

This provides the Gmail sending capability required by the SOC notification workflow.

![Gmail OAuth Send Scope](images/phase13-gmail-oauth-send-scope.png)

OAuth client credentials and authentication tokens are intentionally excluded from this repository.

---

# 8. Gmail API Python Environment

The Gmail API automation requires the Google API Python libraries.

The required packages were installed in a Python virtual environment:

```bash
pip install google-api-python-client google-auth-httplib2 google-auth-oauthlib
```

On the Wazuh server, the dedicated environment was located at:

```text
/opt/phase13-email-venv
```

The libraries were validated with:

```bash
sudo /opt/phase13-email-venv/bin/python -c \
"import googleapiclient; import google_auth_oauthlib; print('Google API libraries OK')"
```

Successful output:

```text
Google API libraries OK
```

---

# 9. Gmail API / OAuth Python Automation

The Gmail API automation uses Google's OAuth libraries rather than storing a Gmail password in the Python source code.

The Gmail send scope is:

```python
SCOPES = ["https://www.googleapis.com/auth/gmail.send"]
```

The OAuth token is loaded separately from the source code:
...
The security notification is submitted through the Gmail API:
...
Gmail Message ID: <generated-message-id>
```

![Gmail API Alert Success](images/phase13-gmail-api-alert-success.png)

The initial Gmail API notification was successfully delivered to the SOC mailbox.
...
# 10. Gmail API Deployment on SOC-Wazuh

After validating the Gmail API automation, the email integration was deployed directly onto the Wazuh Manager.

The integration support directory was:

```text
/var/ossec/integrations/phase13-email/
```

The directory contained the Gmail automation components required by the Wazuh server.

The Gmail API script was manually validated from SOC-Wazuh before enabling automatic Wazuh execution.

Example:

```bash
cd /var/ossec/integrations/phase13-email
```

```bash
/opt/phase13-email-venv/bin/python gmail_api_alert.py
```
...
This confirmed that the Wazuh server itself could successfully communicate with the Gmail API.

---

# 11. Secure Integration Permissions

The OAuth token and automation files were protected within the Wazuh integration directory.
...
The protected OAuth token was stored at:

```text
/var/ossec/integrations/phase13-email/token.json
```

The Wazuh service account was verified to have access to the OAuth token:

```bash
sudo -u wazuh test -r \
/var/ossec/integrations/phase13-email/token.json \
&& echo "WAZUH CAN READ TOKEN"
```

The custom integration was made executable:

```bash
sudo chmod 750 /var/ossec/integrations/custom-phase13-email
```

The Python runtime was also verified under the Wazuh service account:

```bash
sudo -u wazuh /opt/phase13-email-venv/bin/python --version
```
...
# 12. Wazuh-to-Python Integration

The production automation uses the Wazuh custom integration:

```text
/var/ossec/integrations/custom-phase13-email
```

Wazuh passes generated alert information to the integration as JSON.

The Python integration receives the Wazuh-generated alert file as a command-line argument:
...
The integration can then extract information from the Wazuh alert.
...
event = rule.get("description", "N/A")

source_ip = data.get("srcip", "N/A")
destination_ip = data.get("dstip", "N/A")
```

The extracted information is converted into a SOC notification:
...
The message is then processed by the Gmail API automation.

This creates the following automated relationship:
...
# 13. Wazuh Custom Integration Configuration

The custom integration was registered in:
...
```text
Integration: custom-phase13-email
Minimum Alert Level: 5
Alert Format: JSON
```

This allows qualifying Wazuh alerts to be passed automatically to the Python integration.

After configuring the integration, the Wazuh Manager was restarted:

```bash
sudo systemctl restart wazuh-manager
```

The integration service was verified:
...
```bash
sudo pgrep -af wazuh-integratord
```

The Wazuh log confirmed that the integration was loaded:

```bash
sudo grep "Enabling integration for: 'custom-phase13-email'" \
/var/ossec/logs/ossec.log | tail -1
```

Expected result:

```text
wazuh-integratord: INFO: Enabling integration for: 'custom-phase13-email'.
```

---

# 14. Controlled Security Event Validation

A controlled authentication-failure event was generated from the monitored Ubuntu endpoint.
...
Multiple invalid authentication attempts were generated until SSH rejected the login.
...
One generated detection included:
...
Repeated authentication failures also generated a higher-severity Wazuh authentication event.

The Wazuh alert data was reviewed with:

```bash
sudo grep -B 4 -A 8 "fakeuser" \
/var/ossec/logs/alerts/alerts.log | tail -n 30
```

JSON alerts were also reviewed:
...
# 15. Automated Wazuh Email Alert

After Wazuh detected a qualifying security event, `wazuh-integratord` automatically invoked:

```text
custom-phase13-email
```

The Python integration processed the security alert and submitted a notification through the Gmail API.
...
The automatically generated email contained live security-event information including:
...
![Wazuh Automated Email Alert](images/phase13-wazuh-automated-email-alert.png)

This provided final end-to-end validation that a security event detected by the monitoring environment could automatically generate an external SOC notification.

---

# 16. Credential Protection

Authentication secrets are intentionally separated from the Python source code.

The following files and directories must not be committed to GitHub:
...
The Phase 13 `.gitignore` configuration includes:
...
The public repository therefore contains the automation logic and documentation without exposing:
...
Several important operational lessons were identified while implementing Phase 13.

### Custom Wazuh Integration Naming

Wazuh custom integrations require the supported custom integration naming convention.

The final executable was named:

```text
custom-phase13-email
```

This allows `wazuh-integratord` to recognize it as a custom integration.

### Working Directory vs. Absolute Paths

Service-based automation should not depend on an interactive shell's current working directory.

The OAuth token therefore uses an absolute path:
...
Successful execution as an administrator does not automatically prove that the service account can execute the same automation.

Permissions were therefore validated specifically for the Wazuh service account.

### Configuration Validation vs. Functional Validation

A valid configuration does not prove that an automation pipeline works end-to-end.

Phase 13 was validated at multiple layers:
...
The final automatically generated email provided functional proof that the complete security automation pipeline was operational.
...
```text
Suricata / Endpoint Security Events
               |
               v
             Wazuh
               |
...
**COMPLETE — Python Security Automation and Wazuh Automated SOC Email Notification Successfully Validated**

The final implementation demonstrates an end-to-end SOC automation workflow capable of processing security events, detecting qualifying Wazuh alerts, extracting security-event information with Python, and automatically notifying an analyst through an OAuth-authenticated Gmail API integration.
